...
//		$resultCode = $command->run($input, $output);
	 ...
	 *
	 */
	public function checkCallLogAction()
	{

		$em = $this->getDoctrine()->getManager();

		//Get the last calls
		$lastCalls = $em->getRepository('LogBundle:CallLog')->findBy(
			array(),
			array('callDate' => 'DESC'),
			10,
			0
		...
		$lastCalls = array(
			"success" => (!empty($lastCalls)),
			"data" => $lastCalls
		);

		$lastCalls = $serializer->serialize($lastCalls, 'json');

		$response = new Response($lastCalls);
